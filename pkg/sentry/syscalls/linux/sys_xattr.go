...
func getXattr(t *kernel.Task, d *fs.Dirent, dirPath bool, nameAddr usermem.Addr) (string, error) {
	...
	value, err := d.Inode.InodeOperations.GetXattr(t, d.Inode, name)
	if err != nil {
		return "", err
	}
	return value, nil
...
func setXattr(t *kernel.Task, d *fs.Dirent, dirPath bool, nameAddr, valueAddr usermem.Addr, size uint, flags uint32) error {
	...
	buf := make([]byte, size)
	n, err := t.CopyInBytes(valueAddr, buf)
	if err != nil {
		return err
	}
	value := string(buf[:n])
	...
	return d.Inode.InodeOperations.SetXattr(t, d.Inode, name, value, flags)
}
